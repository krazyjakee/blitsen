...
fn endpoint(name: &str) -> Result<Endpoint, PlatformError> {
    ...
    let socket_name = path
        ...
        .map_err(|error| PlatformError::new(format!("could not name {}: {error}", path.display())))?
        .into_owned();
    Ok(Endpoint {
        name: socket_name,
        path,
...
fn endpoint(name: &str) -> Result<Endpoint, PlatformError> {
    ...
    let socket_name = pipe_name
        ...
        .map_err(|error| PlatformError::new(format!("could not name the Windows pipe: {error}")))?
        .into_owned();
    Ok(Endpoint {
        name: socket_name,
        user_sid,
...
fn receive(stream: &mut LocalSocketStream) -> Result<Invocation, String> {
    ...
    read_stream_exact_until(stream, &mut payload, deadline).map_err(|error| error.to_string())?;
...
fn read_acknowledgement(reader: &mut LocalSocketStream, timeout: Duration) -> std::io::Result<()> {
...
mod tests {
    ...
    fn invocation_frames_are_bounded_in_size_and_wall_clock_time() {
        ...
        let error = read_payload(&mut Cursor::new(&oversized), Duration::from_secs(1)).unwrap_err();
        assert_eq!(error.kind(), ErrorKind::InvalidData);

        let error = write_payload(&mut Vec::new(), &oversized, Duration::from_secs(1)).unwrap_err();
        ...
        let error = read_payload(&mut Drip { offset: 0 }, Duration::from_millis(20)).unwrap_err();
