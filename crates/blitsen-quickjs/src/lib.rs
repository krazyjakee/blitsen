...
unsafe extern "C" fn normalize_module(
    ...
    let engine = unsafe { borrowed(ctx) };
    let resolved = unsafe { call_global(ctx, "__blitsenModuleResolve", &[base, name]) };
    match resolved {
        Some(text) => {
        ...
        None => {
            // Leave the pending exception in place: QuickJS reports it as the
            // import failure, which is the message the application should see.
            let _ = engine;
            std::ptr::null_mut()
        }
    }
...
unsafe extern "C" fn load_module(
    ...
    let Some(source) = (unsafe { call_global(ctx, "__blitsenModuleSource", &[name]) }) else {
...
unsafe fn call_global(
    ...
    let mut engine = unsafe { borrowed(ctx) };
    let global = engine.global();
    let function = engine.get_property(&global, name).ok()?;
    let mut values = Vec::with_capacity(arguments.len());
    for argument in arguments {
        let text = if argument.is_null() {
            String::new()
        } else {
            unsafe { CStr::from_ptr(*argument) }.to_string_lossy().into_owned()
        };
        values.push(engine.string(&text).ok()?);
    }
    let result = engine.call(&function, Some(&global), &values).ok()?;
    engine.to_string(&result).ok()
}

/// An engine handle that shares the context without owning it.
unsafe fn borrowed(ctx: *mut q::JSContext) -> QuickJs {
    let state = unsafe { context_state(ctx) };
    QuickJs {
        inner: Rc::new(Inner {
            runtime: state.runtime,
