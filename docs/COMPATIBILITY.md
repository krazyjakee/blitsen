...
`blitsen/{app,window,dialog,clipboard}` modules. Two of its members are partial by design and say so
where they are documented: `dialog.*` is Linux/BSD only, and `window.create` is deliberately absent
