...
| Pointer lock and fullscreen | Desktop pointer lock with raw relative `movementX`/`movementY`; root-element borderless fullscreen on the current monitor; standard state, promises and events — see [Pointer lock and fullscreen](WEB-APIS.md#pointer-lock-and-fullscreen) |
