...
| `notify` | **Present, lifecycle unexercised** | `android-activity` and `jni` bridge the platform `NotificationManager`: a stable default channel, API 33 permission, submission, session-stable replacement IDs and close are implemented. Body taps and action buttons start a minimal `NativeActivity` subclass carrying the activation envelope; its only override retains warm `onNewIntent`. A private receiver in the same small dex persists swipe dismissals without opening the Activity, and Rust delivers every envelope once on a frame turn. The manifest, dex build and persisted handoff are covered deterministically, but system-shade activation and dismissal have not run on an emulator or device (#252). |
