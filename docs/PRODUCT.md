...
| P5c | Platforms, Android | `arm64-v8a` shipping, `x86_64` for emulators | A distinct artifact, not a P5b row: a cross-compiled APK with keystore signing, produced by `blitsen build --android`, not a runtime an install resolves from npm (#148). It is a flag rather than a `--target` value because one APK carries every ABI, and `--target` picks one prebuilt runtime to link. Both defaults ship in one artifact; `armeabi-v7a` builds on request and is unproven. Signed with the Android debug key unless `--android-keystore` names one, and the build says on every run that a debug-signed APK is not distributable. **Not an AAB**, so not a Google Play upload — the packager (`cargo apk`, #139's survey) emits none, and that is the first thing a Gradle backend would buy. The NDK is a prerequisite the CLI detects and never installs: an Android build is a cross-compile, so P9's "no toolchain" does not reach it. Its size budget is its own — P1 is a desktop figure and does not transfer (#150). The API floor is 26, which is not a preference: `cpal` links `libaaudio`, and the NDK ships it from 26 and no earlier. **Evidence: a smoke tier, and a thinner one than P5b's** (#149). CI cross-compiles the entry point for both default ABIs, checks each `.so` is the architecture it claims to be and exports `android_main`, and resolves the third-party notices the artifact owes; the product-behaviour suites stay on the targets that can run them, per #133's line. What CI does *not* do yet is package an APK or boot an emulator, and the reason is not the tier: `blitsen build --android` cannot yet produce an APK containing the engine, because the cdylib it generates invokes a macro `blitsen-android` does not export (#148 is open on exactly this). The emulator harness is written and runnable — `bun run --cwd packages/blitsen test:android` — and the GPU mode it will run under is decided: `-gpu lavapipe`, because #139 measured `swiftshader_indirect` killing the emulator at wgpu initialisation and a hosted runner has no GPU for `-gpu host`. Whether lavapipe carries Vello's compute pipelines is untested by anyone. |
