...
**Status:** Draft v0.1
**Date:** 2026-08-10
**Working name:** Power (provisional; the CLI is written as `power` throughout)

---

## 1. What it is

Power is a **browserless implementation of enough of the web platform to run HTML/CSS/JS
...
Power is principally **the layer that joins them**: the DOM ↔ JS bridge, the web API
...
Power is **not a browser** and does not aspire to be one. It is a native application runtime
...
Honest statement of the trade: **Power will render less of the web than a browser does.** It
wins on binary size versus Electron and on consistency and control versus Tauri, and it loses
on spec coverage against all three. An app targeting Power is authored against Power, not
...
A game is one thing somebody builds with Power, not the product's definition. But it is the
...
1. **The runtime decides as little as possible.** Power supplies the web platform, native
...
2. **Drop into existing projects; never ask for a rewrite.** Power is an export target that
...
### Power is an export target, not a framework to start projects in
...
This is the single most important product decision in the document. **The input to Power is a
directory of static web output**, which makes it bundler- and framework-agnostic by
construction:

```bash
vite build && power build dist      # React, Vue, Svelte, Solid, …
webpack && power build dist
bun build && power build dist
power build .                       # vanilla HTML, no build step at all
```
...
```bash
npx power .                          # open index.html in a native window
npx power http://localhost:5173      # point at a running Vite dev server
```
...
Availability is incremental. A browser having an API does not oblige Power to ship it.
...
`power doctor` reports which web APIs a built bundle references that the runtime does not
...
| P9 | Install | `npm i -D power` fetches only the host platform's runtime | No Rust toolchain, no compile step, no postinstall build. |
...
`npm i -D power` and `power build dist`, and runs. This proves the adoption claim (P10)
independently of the architecture claim, and is the milestone most likely to attract users.

**M4 — Ships.** `npm i -D power` resolves the correct runtime on all six platform targets,
`power build` produces distributable artifacts, and a non-trivial third-party app (an editor or
...
| "Drop-in" invites projects the runtime cannot yet render (Three.js, canvas-heavy apps) | Medium — disappointed first impressions | `power doctor` reports unsupported API usage before the user hits it at runtime; capability tiers published prominently. |
...
1. **Name.** "Power" is the repo directory, not a decision. `native-export` is the leading
   descriptive candidate. Whatever is chosen must work as npm package, CLI verb and
   `@scope/platform` prefix simultaneously — decide before anything is published.
...
5. **Is TypeScript first-class?** Mostly moot under the export model — the user's existing
   bundler handles TS before Power sees the output. Still open for the no-build-step path.
...
7. **Does the dev-server mode ship in v0?** It is the cheapest possible adoption on-ramp
   (`power http://localhost:5173`) and may be worth pulling forward ahead of export.
8. **Native API imports** — `native:dialog` (bare specifier, needs runtime resolver support in
   every bundler) or `power/dialog` (a real npm path that any bundler already resolves)? The
